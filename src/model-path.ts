...
function containedRelative(base: string, candidate: string): string {
  ...
  if (projected === '') throw new Error('resolved document path is the workspace root, not a document')
 ...
 */
export function projectModelPath(requestedPath: string, resolvedDisplayPath: string, cwd?: string): string {
  ...
  if (!isAbsolute(requested)) return posixPath(normalize(requested))
