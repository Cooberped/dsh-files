...
function decodedAttribute(
  ...
  for (const [name, value] of Object.entries(attributes)) {
    ...
    if (localName(name) === wanted) found = decodeEntities(value)
  }
  return found
}

function extractXlsxParts(bytes: Uint8Array, wanted: ReadonlySet<string>): Unzipped {
  try {
...
function worksheetPartNames(bytes: Uint8Array): Set<string> {
  ...
  try {
    ...
    throw new Error(
      `cannot inspect XLSX safely: invalid ${WORKBOOK_RELS_PART}: ` +
...
function positiveDecimal(raw: string, maximum: number, label: string): number {
  const value = raw.trim()
  if (value === '') throw new Error(`${label} is empty`)
  let output = 0
  for (let index = 0; index < value.length; index += 1) {
    const digit = value.charCodeAt(index) - 48
    if (digit < 0 || digit > 9) throw new Error(`${label} ${JSON.stringify(raw)} is not a positive integer`)
  ...
  if (output < 1) throw new Error(`${label} ${JSON.stringify(raw)} must be at least 1`)
  return output
}

function cellCoordinates(raw: string, partName: string): { row: number; column: number } {
  const value = raw.trim()
  let index = 0
  let column = 0
...
function logicalCells(rows: number, columns: number): number {
  return rows === 0 ? 0 : rows * Math.max(columns, 1)
}

function assertSheetGrid(partName: string, rows: number, columns: number): void {
  const cells = logicalCells(rows, columns)
  if (cells > MAX_XLSX_LOGICAL_CELLS_PER_SHEET) {
    throw new Error(
      `cannot inspect XLSX safely: worksheet "${partName}" declares a ${rows}x${Math.max(columns, 1)} ` +
      `logical grid (${cells} cells; limit ${MAX_XLSX_LOGICAL_CELLS_PER_SHEET})`
    )
  }
  if (rows > MAX_XLSX_LOGICAL_ROWS_PER_SHEET) {
    throw new Error(
      `cannot inspect XLSX safely: worksheet "${partName}" declares ${rows} logical rows ` +
      `(limit ${MAX_XLSX_LOGICAL_ROWS_PER_SHEET})`
    )
  }
}

function inspectWorksheetGrid(bytes: Uint8Array, partName: string): number {
  ...
  parser.on(
    ...
    (
      ...
      if (!sheetDataStarted || (element !== 'row' && element !== 'c')) return
      const attributes = getAttributes()
      ...
      if (element === 'row') {
        ...
        if (reference === undefined) throw new Error(`cell without r coordinate in ${partName}`)
  ...
  try {
    ...
    if (error instanceof Error && error.message.startsWith('cannot inspect XLSX safely:')) throw error
    throw new Error(
      `cannot inspect XLSX safely: invalid worksheet XML part "${partName}": ` +
      `${error instanceof Error ? error.message : String(error)}`
    )
  }
...
function assertSafeXlsxLogicalGrid(bytes: Uint8Array): void {
  ...
  for (const partName of worksheetParts) {
    ...
    if (part === undefined) throw new Error(`cannot inspect XLSX safely: missing worksheet part "${partName}"`)
    workbookCells += inspectWorksheetGrid(part, partName)
    if (workbookCells > MAX_TOTAL_XLSX_LOGICAL_CELLS) {
      throw new Error(
        `cannot inspect XLSX safely: workbook declares ${workbookCells} logical cells ` +
