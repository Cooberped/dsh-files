...
interface ProjectedXml {
  text: string
  omittedAltChunk: boolean
}
...
function projectWordXml(bytes: Uint8Array, partName: string): ProjectedXml {
  ...
  parser.on('openTag', (name: string) => {
    if (name === 'w:del' || name === 'w:moveFrom') skippedRevisionDepth += 1
    if (name === 'w:altChunk') omittedAltChunk = true
    if (name === 'w:tr') rowStack.push([])
    if (name === 'w:tc') cellStack.push([])
    if (name === 'w:p') paragraph = []
    if (name === 'w:t') textDepth += 1
    if (paragraph !== null && skippedRevisionDepth === 0) {
      if (name === 'w:tab') paragraph.push('\t')
      if (name === 'w:br' || name === 'w:cr') paragraph.push('\n')
      if (name === 'w:noBreakHyphen') paragraph.push('\u2011')
      if (name === 'w:softHyphen') paragraph.push('\u00ad')
    }
  })
  ...
  parser.on('cdata', (value: string) => {
    if (paragraph !== null && textDepth > 0 && skippedRevisionDepth === 0) paragraph.push(value)
  })

  parser.on('closeTag', (name: string) => {
    if (name === 'w:t') textDepth = Math.max(0, textDepth - 1)
    if (name === 'w:p') {
    ...
    if (name === 'w:tc') {
      ...
      if (row !== undefined) row.push(rendered)
      else appendBlock(rendered)
    }
    if (name === 'w:tr') {
...
function partLabel(name: string): string {
  ...
  if (base === 'footnotes') return 'Footnotes'
  if (base === 'endnotes') return 'Endnotes'
  if (base.startsWith('header')) return `Header ${base.slice('header'.length)}`
  if (base.startsWith('footer')) return `Footer ${base.slice('footer'.length)}`
  return base
}
...
export async function parseDocx(bytes: Uint8Array): Promise<string> {
  ...
  if (main === undefined) throw new Error(`invalid DOCX package: missing ${MAIN_PART}`)

  const output: string[] = []
  let omittedAltChunk = false
  const body = projectWordXml(main, MAIN_PART)
  omittedAltChunk ||= body.omittedAltChunk
  if (body.text !== '') output.push(body.text)
  ...
  for (const name of optionalNames) {
    ...
    if (projected.text !== '') output.push(`### ${partLabel(name)}\n${projected.text}`)
  }
