...
function parseXml(bytes: Uint8Array, partName: string, configure: (parser: Parser) => void): void {
...
function relationships(bytes: Uint8Array, partName: string): Map<string, Relationship> {
  const output = new Map<string, Relationship>()
  parseXml(bytes, partName, (parser) => {
    parser.ns({ [PACKAGE_REL_NS]: 'rel' })
    parser.on('openTag', (name: string, attrGetter: () => Record<string, string>) => {
      if (name !== 'rel:Relationship') return
      ...
      if (output.has(id)) throw new Error(`duplicate relationship ${id} in ${partName}`)
...
function presentationSlideIds(bytes: Uint8Array): string[] {
  const output: string[] = []
  parseXml(bytes, PRESENTATION_PART, (parser) => {
    ...
    parser.on('openTag', (name: string, attrGetter: () => Record<string, string>) => {
      ...
      if (typeof id !== 'string' || id === '') throw new Error(`slide entry without r:id in ${PRESENTATION_PART}`)
      output.push(id)
    })
  })
  return output
...
function projectDrawingText(bytes: Uint8Array, partName: string): string {
  const lines: string[] = []
  ...
  parseXml(bytes, partName, (parser) => {
    ...
    parser.on('openTag', (name: string) => {
      if (name === 'a:p') paragraph = []
      if (name === 'a:t') textDepth += 1
      if (paragraph !== null && name === 'a:br') paragraph.push('\n')
      if (paragraph !== null && name === 'a:tab') paragraph.push('\t')
    })
    parser.on('text', (value: string, decodeEntities: (text: string) => string) => {
      if (paragraph !== null && textDepth > 0) paragraph.push(decodeEntities(value))
    })
    parser.on('cdata', (value: string) => {
      if (paragraph !== null && textDepth > 0) paragraph.push(value)
    })
    parser.on('closeTag', (name: string) => {
      if (name === 'a:t') textDepth = Math.max(0, textDepth - 1)
      if (name !== 'a:p') return
      const cleaned = cleanParagraph(paragraph?.join('') ?? '')
      if (cleaned !== '') lines.push(...cleaned.split('\n'))
      paragraph = null
    })
  })
  return lines.join('\n').replace(/\n{3,}/g, '\n\n').trim()
...
function notePartForSlide(parts: Unzipped, slidePart: string): string | undefined {
  const relsPart = relationshipsPart(slidePart)
  const relsBytes = parts[relsPart]
  if (relsBytes === undefined) return undefined
...
export async function parsePptx(bytes: Uint8Array): Promise<string> {
  ...
  if (presentationRels === undefined) throw new Error(`invalid PPTX package: missing ${PRESENTATION_RELS_PART}`)
  ...
  for (const [index, slide] of orderedSlides.entries()) {
    ...
    if (notePart !== undefined) {
      ...
      if (noteText !== '') section.push('#### Speaker notes', noteText)
    }
