// dsh-docs — single dual-face plugin replacing dsh-upload-button and
// dsh-plugin-doc-reader. One cordis row, one apply, two capabilities:
...
export const Config = z.object({
    ...
    /** Upload storage root; files land in <root>/.dsh-uploads/<sessionId>/. */
    uploadDir: z.string().default(join(process.cwd(), 'uploads'))
});
function assertPositiveInteger(value, label) {
    if (!Number.isInteger(value) || value < 1)
        throw new Error(`dsh-docs: ${label} must be a positive integer`);
}
export function apply(ctx, config) {
    ...
    const cache = new ParseCache(config.cacheEntries);
