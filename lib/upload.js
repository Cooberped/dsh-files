...
//     official dsh-upload-button contract)
...
export function createUploadHandler(options) {
    ...
    async function handlePost(req, res) {
        ...
        try {
            ...
            console.error('[dsh-docs] upload persist failed:', err);
...
export function createSweeper(root, ttlMs, intervalMs, now = () => Date.now()) {
    ...
    const timer = setInterval(() => {
        void sweep(root, ttlMs, now).catch((err) => {
            console.error('[dsh-docs] upload sweep failed:', err);
        });
    }, intervalMs);
    timer.unref?.();
...
export async function sweep(root, ttlMs, now = () => Date.now()) {
    ...
    let removedFiles = 0;
    let removedDirs = 0;
    // Uploaded files live at <root>/.dsh-uploads/<sessionId>/; session dirs are
    // the only entries directly under the uploads base.
    const base = join(root, '.dsh-uploads');
    let sessions;
    try {
