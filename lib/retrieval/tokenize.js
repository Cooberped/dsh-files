...
export function buildQueryPlan(value) {
    ...
    // For a single longer CJK phrase, an exact phrase miss may still benefit
    // from a bounded bigram OR retry. Multi-segment queries retain whole phrase
    // groups so ASCII words and short CJK phrases do not dissolve into noise.
    const relaxedGroups = phrases.length === 1 && phrases[0].length >= 3
        ? phrases[0].map((token) => [token])
        : phrases;
    return {
        normalizedQuery,
        phrases,
        ...
        relaxedFtsQuery: relaxedGroups.map(quoteFtsPhrase).join(' OR ')
    };
}
