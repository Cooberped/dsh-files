...
function positiveCharacterRange(start, end) {
    const startChar = Number(start);
    const endChar = Number(end);
    if (!Number.isInteger(startChar) || !Number.isInteger(endChar) || startChar < 1 || endChar < startChar)
        return null;
...
export function parseDocumentLocator(coordinate, sheetIndexes) {
    ...
    if (pageChars !== null) {
        ...
        if (pageNumber < 1 || lineNumber < 1 || chars === null)
            return null;
    ...
    if (slideChars !== null) {
        ...
        if (slideNumber < 1 || lineNumber < 1 || chars === null)
            return null;
    ...
    if (charMatch !== null && charRange === null)
        return null;
...
function splitTextRanges(value, maxChars) {
 ...
 */
export function renderedSpreadsheetRows(projection) {
    ...
    sheets: for (const sheet of sheets) {
        ...
        for (const { rowNumber, text: rendered, lastColumn, columnHeading } of rows) {
            ...
            for (const part of parts) {
                if (!appendBlock(state, document, parts.length === 1 ? coordinate : `${coordinate},chars:${part.startChar}-${part.endChar}`, heading, part.text, options.maxBlocks))
                    break sheets;
                await maybeYieldAfterBlock(state, options.signal);
            }
        }
