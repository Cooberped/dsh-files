...
import { MemoryRetrievalBackend, SqliteRetrievalBackend } from "./backend.js";
...
function errorCategory(error) {
    ...
    if (message === 'FTS5 ordered bigram phrase probe failed')
        return 'FTS5_PHRASE_PROBE_FAILED';
    if (message.includes('expected mode 0700'))
        return 'INDEX_DIRECTORY_PERMISSIONS';
    if (message === 'retrieval index path is not a directory')
        return 'INDEX_DIRECTORY_TYPE';
    if (error instanceof TypeError)
        return 'TYPE_ERROR';
    return 'UNKNOWN_ERROR';
}
/** Stable, actionable, model-safe fallback reason: never embeds error text or host paths. */
function modelSafeFallbackReason(error, scope) {
...
async function internalProbe(loadSqlite) {
    ...
    try {
        ...
        try {
            ...
            const compileRows = db.prepare('PRAGMA compile_options').all();
      ...
      `);
            ...
            const reversed = db.prepare('SELECT rowid FROM retrieval_probe WHERE retrieval_probe MATCH ?')
                .get('"绩效 效流 流程"');
            if (ordered === undefined || reversed !== undefined)
                throw new Error('FTS5 ordered bigram phrase probe failed');
            return {
                sqlite,
                report: {
                    nodeVersion,
...
export async function probeRetrievalRuntime(loadSqlite = () => import('node:sqlite')) {
    return (await internalProbe(loadSqlite)).report;
}
async function secureIndexDirectory(indexDir) {
    await mkdir(indexDir, { recursive: true, mode: 0o700 });
    const info = await stat(indexDir);
    if (!info.isDirectory())
        throw new Error('retrieval index path is not a directory');
...
async function secureSqliteCompanions(dbPath) {
...
export async function createRetrievalBackend(options) {
    ...
    options.logger.info('retrieval self-check: node=%s backend=%s sqlite=%s compileOptionFts5=%s phraseProbe=%s', report.nodeVersion, report.backend, report.sqliteVersion ?? 'unavailable', report.compileOptionFts5 ?? false, report.phraseProbe);
