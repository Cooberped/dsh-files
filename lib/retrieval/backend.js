...
function normalizedSearchText(block) {
    return `${block.heading}\n${block.coordinate}\n${block.text}`.normalize('NFKC');
}
function supportsRelaxedFallback(plan) {
...
export class MemoryRetrievalBackend {
    ...
    syncDocumentDescriptor(document, now) {
        const stored = this.documents.get(document.id);
        if (stored === undefined || stored.descriptor.version !== document.version)
            return;
    ...
    async replaceDocumentCooperatively(document, blocks, now) {
        const staged = [];
    ...
    search(plan, documentIds, limit) {
            ...
            for (const documentId of documentIds) {
                ...
                for (const block of document.blocks) {
                    ...
                    if (mode === 'strict') {
                        ...
                        matchesPhrases = phrase.some((token) => block.headingTokens.includes(token) || block.textTokens.includes(token));
                    }
                    else {
        ...
        if (strict.length > 0 || !supportsRelaxedFallback(plan))
            return strict;
...
export class SqliteRetrievalBackend {
    ...
    replaceDocument(document, blocks, now) {
      ...
      `);
            for (const block of blocks) {
                insertBlock.run(block.id, document.id, document.version, block.ordinal, block.coordinate, block.heading, block.text, normalizedSearchText(block));
    ...
    ftsCandidates(matchExpression, documentIds, candidateLimit, singleCharacters = []) {
      ...
      LIMIT ?
    `;
        const rows = this.db.prepare(sql).all(matchExpression, ...documentIds, ...characterValues, candidateLimit);
        return rows.map((row) => ({
