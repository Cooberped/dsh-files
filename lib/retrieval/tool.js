...
function modelVisiblePath(input, cwd) {
    const normalized = input.normalize('NFC');
    if (cwd === undefined || !isAbsolute(normalized))
        return normalized;
...
async function readSource(ctx, path, cwd, config, exec, sourceCache) {
    ...
    if (format === null) {
        ctx.emit('fs/observed', target, { kind: 'present', version: info.version }, exec);
        throw new FsError(`cannot index "${target.displayPath}": unrecognized file content (expected text, PDF, DOCX, XLSX or PPTX)`, 'FS_NOT_TEXT');
    }
    const descriptor = {
