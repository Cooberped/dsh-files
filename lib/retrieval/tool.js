...
import { buildDocumentBlocks, documentBlockBuildMetadata, INDEX_TRUNCATION_MARKER, parseDocumentLocator, parseWorkbookInventory, retrievalDocumentVersion } from "./blocks.js";
...
async function readSource(ctx, path, cwd, workspaceRoot, config, exec, sourceCache) {
    ...
    if (info.type !== 'file')
        throw new FsError(`cannot index "${modelPath}": not a regular file`, 'FS_NOT_REGULAR_FILE');
    if (info.size !== undefined && info.size > config.maxFileBytes) {
        ctx.emit('fs/observed', target, { kind: 'present', version: info.version }, exec);
        throw new FsError(`cannot index "${modelPath}": file is ${info.size} bytes, over the ${config.maxFileBytes} byte limit`, 'FS_TOO_LARGE');
    }
    ...
    if (format === null) {
        ctx.emit('fs/observed', target, { kind: 'present', version: info.version }, exec);
        throw new FsError(`cannot index "${modelPath}": unrecognized file content (expected text, PDF, DOCX, XLSX or PPTX)`, 'FS_NOT_TEXT');
    }
    const descriptor = {
...
function locatorOutput(locator) {
    if (locator === null)
        return { kind: 'unknown' };
...
export function renderSearchDocumentsResult(value) {
    const backendNotice = value.backendNotice === undefined ? [] : [`NOTICE: ${value.backendNotice}`];
    const warnings = value.truncatedDocuments.map((document) => `WARNING: ${document.path} index was truncated at ${document.maxBlocks} blocks [version ${document.version}]. Later content may require read_document coordinate expansion or sequential paging.`);
    if (value.mode === 'index') {
        const header = `### document index — ${value.documents.length} document(s); backend ${value.backend}; newly indexed ${value.indexedDocuments}`;
        const documents = value.documents.map((document, index) => `${index + 1}. ${document.path} (${document.format}) [version ${document.version}]`);
        return [
            header,
            ...backendNotice,
            ...documents,
    ...
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            for (const result of results) {
                ...
                if (known?.version === result.version)
                    continue;
            ...
            return {
                ...
                results: results.map(({ documentId, ...result }) => ({
                    ...result,
                    locator: locatorOutput(parseDocumentLocator(result.coordinate, indexedMetadata.get(documentId)?.sheetIndexes))
                }))
            };
        },
        presentCall(args) {
