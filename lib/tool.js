...
export function defineReadDocumentTool(ctx, config, cache) {
    return defineTool({
        name: 'read_document',
        description: 'Read text/PDF/DOCX/XLSX without Python. For XLSX, call list_sheets first, then select sheet and optionally cell_range (A1 notation). Results report detected value counts and truncation explicitly.',
        parameters: {
            file_path: {
                type: 'string',
                required: true,
