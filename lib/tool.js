...
import { parseDocumentLocator, parseWorkbookInventory, retrievalDocumentVersion, splitPptxSlides } from "./retrieval/blocks.js";
    ...
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            if (input.version !== undefined && input.version !== version) {
                ctx.emit('fs/observed', target, { kind: 'present', version: info.version }, exec);
                throw new Error(`cannot expand coordinate for "${target.displayPath}": requested version ${input.version}, current version ${version}; rerun search_documents and use its new coordinate/version`);
            }
