...
import { sniffFormat, SUPPORTED_FORMATS } from "./detect.js";
...
export function defineReadDocumentTool(ctx, config, cache) {
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            const format = sniffFormat(bytes, input.format === 'auto' ? undefined : input.format);
            if (format === null) {
            ...
            if (text === undefined) {
                text = await parseDocument(bytes, format, { sheetRowLimit: config.sheetRowLimit });
