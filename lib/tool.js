...
function parseArgs(args, config) {
    ...
    return { filePath, offset, limit, format: format, sheet, listSheets, cellRange };
}
...
export function defineReadDocumentTool(ctx, config, cache) {
    return defineTool({
        name: 'read_document',
        description: 'Read text/PDF/DOCX/XLSX/PPTX without Python. PPTX preserves slide order and speaker notes. For XLSX, call list_sheets first, then select sheet and optionally cell_range (A1 notation). Results report detected value counts and truncation explicitly.',
        parameters: {
            file_path: {
                type: 'string',
                required: true,
        ...
        async execute(args, exec) {
            ...
            if (headFormat === null && input.format === 'auto') {
                ctx.emit('fs/observed', target, { kind: 'present', version: info.version }, exec);
                throw new FsError(`cannot read "${target.displayPath}": unrecognized file content (expected text, PDF, DOCX, XLSX or PPTX)`, 'FS_NOT_TEXT');
            }
            ...
            if ((input.sheet !== undefined || input.listSheets || input.cellRange !== undefined) && format !== 'xlsx') {
                ctx.emit('fs/observed', target, { kind: 'present', version: info.version }, exec);
                throw new FsError(`cannot read "${target.displayPath}": sheet/list_sheets/cell_range parameters are only supported for XLSX files (detected format: ${format})`, 'FS_NOT_TEXT');
            }
            const cacheKey = {
