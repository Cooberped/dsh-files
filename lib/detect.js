...
export const SUPPORTED_FORMATS = new Set(['pdf', 'docx', 'xlsx', 'pptx', 'text']);
...
export function zipMembers(bytes) {
    ...
    if (diskNumber !== 0 || centralDirectoryDisk !== 0 || entriesOnDisk !== count ||
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (nameLen === 0 || nameLen > MAX_ZIP_MEMBER_NAME_BYTES || next > centralDirectoryEnd)
            return null;
