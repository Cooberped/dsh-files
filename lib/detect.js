...
export function zipMemberNames(bytes) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (off + 46 + nameLen > len)
            return null;
        // Names are CP437 in the archive; ASCII member prefixes are all we probe.
        names.push(String.fromCharCode(...bytes.subarray(off + 46, off + 46 + nameLen)));
